...
{
    public class UseWorkCodeLogic : IAttendanceLogic
    {
        #region ClassMembers
        ...
        public UseWorkCodeLogic(EnrolledEmployee emp, IEnumerable<RawData> rData,  DateTime dateToProcess)
        {
            this.employee = emp;
        ...
        public void processAttendance(enShiftType shiftType,string tableName = "")
        {
            try
            {
                ...
                IEnumerable<int> inCodes, outCodes, brkinCodes, brkoutCodes;
                inCodes = _unitOfWork.WorkCodes.All().Where(x => x.type == (int)enStatusCodes.TimeIn).Select(x => x.code).ToList();
                outCodes = _unitOfWork.WorkCodes.All().Where(x => x.type == (int)enStatusCodes.TimeOut).Select(x => x.code).ToList();
                brkoutCodes = _unitOfWork.WorkCodes.All().Where(x => x.type == (int)enStatusCodes.BreakOut).Select(x => x.code).ToList();
                brkinCodes = _unitOfWork.WorkCodes.All().Where(x => x.type == (int)enStatusCodes.BreakIn).Select(x => x.code).ToList();
                //bool found = false;
                ...
                else if (shiftType == enShiftType.Dynamic)
                {
                    ...
                    if (!Common.Common._useRoster)
                    {
                        ...
                        {
                            if (inCodes.Contains(rawD.R_WORK))
                            {
